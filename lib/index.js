...
import { ohDshStatus, applyOhDshUpdate, applyOhDshState, downloadDesktopInstaller, runDesktopInstaller } from "./ohdsh-update.js";
...
async function handle(req, res) {
	...
	if (req.method === "POST" && path === "/dsh-autoupdate/ohdsh/apply") {
		...
		// job pattern: return immediately, poll /apply-state — the swap+boot
		// flow outruns any sane HTTP timeout
		json(res, await applyOhDshUpdate(body.confirmDelete === true));
		return;
	}
	if ((req.method === "GET" || req.method === "HEAD") && path === "/dsh-autoupdate/ohdsh/apply-state") {
		json(res, applyOhDshState());
		return;
	}
	// (applyOhDshState reads the state file; safe across restarts)
