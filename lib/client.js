...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			"au.ohdsh.confirmDelete": "以下第三方插件与新版本不兼容，禁用后仍无法启动。是否删除这些插件并重试更新？（数据目录不受影响）",
			"au.ohdsh.cancelledKeep": "已取消更新，旧版本继续运行",
		...
		var en = {
			...
			"au.ohdsh.confirmDelete": "These third-party plugins are incompatible with the new version and could not be disabled successfully. Delete them and retry the update? (Your data directory is not affected)",
			"au.ohdsh.cancelledKeep": "Update cancelled; the previous version keeps running",
		...
		function OhDshCard(props) {
			...
			function pollState() {
				fetch("/dsh-autoupdate/ohdsh/apply-state", { cache: "no-store" })
					.then(function (r) { return r.json(); })
					.then(function (s) {
						if (!s.done) {
							setNote("… " + (s.phase || "working"));
							setTimeout(pollState, 3000);
						} else {
							var d = s.result || {};
							if (d.ok && d.noop) setNote(t("au.ohdsh.uptodate"));
							else if (d.ok) {
								var parts = ["✓ " + (d.version || "")];
								if (d.disabled && d.disabled.length) parts.push("已禁用: " + d.disabled.join(", "));
								if (d.deleted && d.deleted.length) parts.push("已删除: " + d.deleted.join(", "));
								setNote(parts.join("  |  "));
							} else if (d.needsConfirmation) {
								var msg = t("au.ohdsh.confirmDelete") + "\n\n" + (d.plugins || []).join("\n");
								if (window.confirm(msg)) { setBusy(false); updateWeb(true); return; }
								setNote("✗ " + t("au.ohdsh.cancelledKeep") + "  (" + (d.plugins || []).join(", ") + ")");
							} else setNote("✗ " + (d.error || "unknown") + (d.rolledBack ? " → " + t("au.ohdsh.uptodate") + " (rollback)" : ""));
							setBusy(false);
							refresh();
						}
					})
					.catch(function () { setTimeout(pollState, 5000); }); // DSH restarting mid-update: keep polling
			}
			function updateWeb(confirmDelete) {
				if (!confirmDelete && !window.confirm(t("au.ohdsh.updating"))) return;
				setBusy(true); setNote(t("au.ohdsh.updating"));
				fetch("/dsh-autoupdate/ohdsh/apply", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({ confirmDelete: confirmDelete === true })
				})
					.then(function (r) { return r.json(); })
					.then(function (d) {
						if (d.started) { pollState(); }
						else { setNote(d.reason || "not started"); setBusy(false); }
					})
					.catch(function (e) { pollState(); }); // connection may drop when DSH restarts — poll wins anyway
			}
